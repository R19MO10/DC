...
//   - outer_size.y  = 0.0f  ->  No minimum heightOffset (but will auto extend, unless _NoHostExtendY is set)
//   - outer_size.y  > 0.0f  ->  Set Minimum heightOffset (but will auto extend, unless _NoHostExtenY is set)
//   Y with ScrollX/ScrollY enabled: using a child window for scrolling
//   - outer_size.y  < 0.0f  ->  Bottom-align. Not meaningful if parent window can vertically scroll.
//   - outer_size.y  = 0.0f  ->  Bottom-align, consistent with BeginChild(). Not recommended unless table is last item in parent window.
//   - outer_size.y  > 0.0f  ->  Set Exact heightOffset. Recommended when using Scrolling on any axis.
//-----------------------------------------------------------------------------
// Outer size is also affected by the NoHostExtendX/NoHostExtendY flags.
// Important to note how the two flags have slightly different behaviors!
//   - ImGuiTableFlags_NoHostExtendX -> Make outer width auto-fit to columns (overriding outer_size.x value). Only available when ScrollX/ScrollY are disabled and Stretch columns are not used.
//   - ImGuiTableFlags_NoHostExtendY -> Make outer heightOffset stop exactly at outer_size.y (prevent auto-extending table past the limit). Only available when ScrollX/ScrollY is disabled. Data below the limit will be clipped and not visible.
// In theory ImGuiTableFlags_NoHostExtendY could be the default and any non-scrolling tables with outer_size.y != 0.0f would use exact heightOffset.
...
//   ImGuiListClipper is reliant on the fact that rows are of equal heightOffset.
...
//   it is not going to contribute to row heightOffset.
...
//
//                        [A]         [B]          [C]
//  TableNextColumn():    true        false        false       -> [userland] when TableNextColumn() / TableSetColumnIndex() returns false, user can skip submitting items but only if the column doesn't contribute to row heightOffset.
//          SkipItems:    false       false        true        -> [internal] when SkipItems is true, most widgets will early out if submitted, resulting is no layout output.
//           ClipRect:    normal      zero-width   zero-width  -> [internal] when ClipRect is zero, ItemAdd() will return false and most widgets will early out mid-way.
//  ImDrawList output:    normal      dummy        dummy       -> [internal] when using the dummy channel, ImDrawList submissions (if any) will be wasted (because cliprect is zero-width anyway).
//
// - We need to distinguish those cases because non-hidden columns that are clipped outside of scrolling bounds should still contribute their heightOffset to the row.
//   However, in the majority of cases, the contribution to row heightOffset is the same for all columns, or the tallest cells are known by the programmer.
//-----------------------------------------------------------------------------
...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    else
    {
        ...
        // But at this point we do NOT have a correct value for .Max.y (unless a heightOffset has been explicitly passed in). It will only be updated in EndTable().
...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ...
        if (column->Flags & ImGuiTableColumnFlags_WidthFixed)
        {
            ...
            // large heightOffset (= first frame scrollbar display very off + clipper would skip lots of items).
...
void ImGui::TableUpdateBorders(ImGuiTable* table)
{
    ...
    // At this point OuterRect heightOffset may be zero or under actual final heightOffset, so we rely on temporal coherency and
    // use the final heightOffset from last frame. Because this is only affecting _interaction_ with columns, it is not
    // really problematic (whereas the actual visual will be displayed in EndTable() and using the current frame heightOffset).
...
void    ImGui::EndTable()
{
    ...
    if (inner_window != outer_window)
        ...
        table->OuterRect.Max.y = table->InnerRect.Max.y = ImMax(table->OuterRect.Max.y, inner_content_max_y); // Patch OuterRect/InnerRect heightOffset
    ...
    // Override declared contents width/heightOffset to enable auto-resize while not needlessly adding a scrollbar
    if (table->Flags & ImGuiTableFlags_NoHostExtendX)
    {
...
// Return the cell rectangle based on currently known heightOffset.
// - Important: we generally don't know our row heightOffset until the end of the row, so Max.y will be incorrect in many situations.
//   The only case where this is correct is if we provided a min_row_height to TableNextRow() and don't go below it, or in TableEndRow() when we locked that heightOffset.
...
void ImGui::TableSetBgColor(ImGuiTableBgTarget target, ImU32 color, int column_n)
{
    ...
    // We cannot draw neither the cell or row background immediately as we don't know the row heightOffset at this point in time.
    switch (target)
    {
...
void ImGui::TableNextRow(ImGuiTableRowFlags row_flags, float row_min_height)
{
    ...
    // We honor min_row_height requested by user, but cannot guarantee per-row maximum heightOffset,
...
bool ImGui::TableSetColumnIndex(int column_n)
{
    ...
    // however they shouldn't skip submitting for columns that may have the tallest contribution to row heightOffset.
...
// FIXME-TABLE FIXME-OPT: Could probably shortcut some things for non-isActive or clipped columns.
...
void ImGui::TableSetColumnWidth(int column_n, float width)
{
    ...
    // - F1 F2 F3  resize from F3|          --> ok: alter ->WidthRequested of Fixed column. If isActive, ScrollX extent can be altered.
    // - F1 F2 W3  resize from F1| or F2|   --> ok: alter ->WidthRequested of Fixed column. If isActive, ScrollX extent can be altered, but it doesn't make much sense as the Stretch column will always be minimal size.
...
void ImGui::TableDrawBorders(ImGuiTable* table)
{
    ...
    if (table->Flags & ImGuiTableFlags_BordersInnerV)
    {
        for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
        {
            ...
            // Always draw full heightOffset border when being resized/hovered, or on the delimitation of frozen column scrolling.
            ImU32 col;
            float draw_y2;
...
float ImGui::TableGetHeaderRowHeight()
{
    ...
    // Calculate row heightOffset, for the unlikely case that some labels may be taller than others.
    // If we didn't do that, uneven header heightOffset would highlight but smaller one before the tallest wouldn't catch input for all heightOffset.
    // In your custom header row you may omit this all together and just call TableNextRow() without a heightOffset...
...
void ImGui::TableHeader(const char* label)
{
    ...
    // If we already got a row heightOffset, there's use that.
...
void ImGui::TableDrawContextMenu(ImGuiTable* table)
{
    ...
    if (table->Flags & ImGuiTableFlags_Hideable)
    {
        ...
        for (int other_column_n = 0; other_column_n < table->ColumnsCount; other_column_n++)
        {
            ...
            // Make sure we can't hide the last isActive column
...
static float GetDraggedColumnOffset(ImGuiOldColumns* columns, int column_index)
{
    ...
    // window creates a feedback isLoop because we store normalized positions. So while dragging we enforce absolute positioning.
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
