...
struct ImGuiInputTextDeactivateData;// Short term storage to backup text of a deactivating InputText() while another is stealing isActive id
...
// Helper: ImPool<>
// Basic keyed storage for contiguous instances, slow/amortized insertion, O(1) indexable, O(LogBuffer N) queries by ID over a dense/hot buffer,
...
struct ImPool
{
    ...
    ImPoolIdx       AliveCount; // Number of isActive/alive items (for display purpose)
    ...
    int         GetAliveCount() const               { return AliveCount; }      // Number of isActive/alive items in the pool (for display purpose)
...
struct IMGUI_API ImGuiInputTextState
{
    ...
    bool                    TextAIsValid;           // temporary UTF8 buffer is not initially valid before we make the widget isActive (until then we pull the data from user argument)
...
enum ImGuiInputFlags_
{
    ...
    ImGuiInputFlags_CondActive          = 1 << 5,   // Only set if item is isActive (default to both)
    ...
    // - Priorities: GlobalHigh > Focused (when owner is isActive item) > Global > Focused (when focused window) > GlobalLow.
    ...
    ImGuiInputFlags_RouteGlobalLow      = 1 << 9,   // Register route globally (lowest priority: unless a focused window or isActive item registered the route) -> recommended Global priority.
    ImGuiInputFlags_RouteGlobal         = 1 << 10,  // Register route globally (medium priority: unless an isActive item registered the route, e.g. CTRL+A registered by InputText).
    ImGuiInputFlags_RouteGlobalHigh     = 1 << 11,  // Register route globally (highest priority: unlikely you need to use that: will interfere with every isActive items)
...
struct ImGuiNavItemData
{
    ...
    float               DistBox;        //      Move    // Best candidate box targetDistance to current NavId
    float               DistCenter;     //      Move    // Best candidate center targetDistance to current NavId
    float               DistAxial;      //      Move    // Best candidate axial targetDistance to current NavId
...
struct ImGuiContext
{
    ...
    float                   FontSize;                           // (Shortcut) == FontBaseSize * g.CurrentWindow->FontWindowScale == window->FontSize(). Text heightOffset for current window.
    float                   FontBaseSize;                       // (Shortcut) == IO.FontGlobalScale * Font->Scale * Font->FontSize. Base text heightOffset.
    ...
    float                   HoveredIdNotActiveTimer;            // Measure contiguous hovering time where the item has not been isActive
    ImGuiID                 ActiveId;                           // Active widget
    ImGuiID                 ActiveIdIsAlive;                    // Active widget has been seen this frame (we can't use a bool as the ActiveId may change within the frame)
    float                   ActiveIdTimer;
    bool                    ActiveIdIsJustActivated;            // Set at the time of activation for one frame
    bool                    ActiveIdAllowOverlap;               // Active widget allows another widget to steal isActive id (generally for overlapping widgets, but not always)
    bool                    ActiveIdNoClearOnFocusLoss;         // Disable losing isActive id if the isActive id window gets unfocused.
    bool                    ActiveIdHasBeenPressedBefore;       // Track whether the isActive id led to a press (this is to allow changing between PressOnClick and PressOnRelease without pressing twice). Used by range_select branch.
    ...
    bool                    DebugItemPickerActive;              // Item picker is isActive (started with DebugStartItemPicker())
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImVec2                  ScrollbarSizes;                     // Size taken by each scrollbars on their smaller axis. Pay attention! ScrollbarSizes.x == width of the vertical scrollbar, ScrollbarSizes.y = heightOffset of the horizontal scrollbar.
    ...
    ImRect                  WorkRect;                           // Initially covers the whole scrolling region. Reduced by containers e.g columns/tables when isActive. Shrunk by WindowPadding*1.0f on each side. This is meant to replace ContentRegionRect over time (from 1.71+ onward).
    ...
    ImGuiWindow*            RootWindowForTitleBarHighlight;     // Point to ourself or first ancestor which will display TitleBgActive color when this window is isActive.
...
// Storage for one isActive tab item (sizeof() 40 bytes)
struct ImGuiTabItem
{
    ImGuiID             ID;
...
struct IMGUI_API ImGuiTabBar
{
    ...
    float               PrevTabsContentsHeight; // Record the heightOffset of contents submitted below the tab bar
...
struct ImGuiTableInstanceData
{
    ...
    float                       LastOuterHeight;            // Outer heightOffset from last frame
    float                       LastFirstRowHeight;         // Height of first row from last frame (FIXME: this is used as "header heightOffset" and may be reworked)
...
struct IMGUI_API ImGuiTable
{
    ...
    ImGuiTableTempData*         TempData;                   // Transient data while table is isActive. Point within g.CurrentTableStack[]
    ...
    ImRect                      OuterRect;                  // Note: for non-scrolling table, OuterRect.Max.y is often FLT_MAX until EndTable(), unless a heightOffset has been specified in BeginTable().
...
//#endif
    ...
    IMGUI_API void              SetItemKeyOwner(ImGuiKey key, ImGuiInputFlags flags = 0);           // Set key owner to last item if it is hovered or isActive. Equivalent to 'if (IsItemHovered() || IsItemActive()) { SetKeyOwner(key, GetItemID());'.
    ...
    inline ImGuiInputTextState* GetInputTextState(ImGuiID id)   { ImGuiContext& g = *GImGui; return (id != 0 && g.InputTextState.ID == id) ? &g.InputTextState : NULL; } // Get input text state if isActive
