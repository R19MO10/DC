...
public:
  ...
  */
  ...
  {
    const float left = std::clamp(0.5f - pan / 2, 0.0f, 1.0f);
    const float right = std::clamp(0.5f + pan / 2, 0.0f, 1.0f);
...
vec3 listnerPosition = { 0, 0, 0 };
vec3 listnerRight = { 1, 0, 0 };
...
void SetPanAndVolumeFromPosition(int playerId, const vec3& position, float volume)
{
  ...
  vec3 v = position - listnerPosition;
...
*/
void PlaySound2D(
    const char* filename, float volume, float pitch)
{
    if (!engine) {
        return;
    }
    ...
    const Attenuation* attenuation)
{
    ...
    // アテネーションを取得する
    const Attenuation& attena =
        (attenuation ? *attenuation : Attenuation());

    const float len = length(position - listnerPosition);
    if (len > attena.innerRadius + attena.falloffDistance) {
        // 距離が減衰距離以上の場合、音量が0のため再生しない
        return;
    }
    ...
    if (len > attena.innerRadius) {
        // 音量を減衰させる
        volume *= (1.0f - (len - attena.innerRadius) / attena.falloffDistance);
    }
    p->SetVolume(volume);

    //const vec3 direc = normalize(position - listnerPosition); 
    //// 方向ベクトルと右方向ベクトルの内積を計算
    //const float pan = dot(direc, listnerRight);
    //p->SetPan(pan);
